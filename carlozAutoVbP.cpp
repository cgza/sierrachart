...
SCSFExport scsf_AutoVbP(SCStudyInterfaceRef sc)
{
    ...
    int targetTicksPerBar = max(sc.Round(vDiff / detail), 1);
	...
	//if targetTicksPerBar is less than the step, then modify it as a VAP multiple
	else
	{
		targetTicksPerBar -= targetTicksPerBar % sc.VolumeAtPriceMultiplier;
		if(targetTicksPerBar < sc.VolumeAtPriceMultiplier) targetTicksPerBar = sc.VolumeAtPriceMultiplier;
		if(targetTicksPerBar < 1) targetTicksPerBar = 1;
	}
